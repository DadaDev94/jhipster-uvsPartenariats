...
import com.aida.uvspartenariats.domain.Country;
import com.aida.uvspartenariats.repository.CountryRepository;
import com.aida.uvspartenariats.repository.search.CountrySearchRepository;
import com.aida.uvspartenariats.service.CountryService;
import com.aida.uvspartenariats.service.dto.CountryDTO;
import com.aida.uvspartenariats.service.mapper.CountryMapper;
...
public class CountryResourceIT {
     ...
     *
     * @see com.aida.uvspartenariats.repository.search.CountrySearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void deleteCountry() throws Exception {
        ...
        // Delete the country
        restCountryMockMvc
            .perform(delete("/api/countries/{id}", country.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
    ...
    public void searchCountry() throws Exception {
        ...
        // Search the country
        restCountryMockMvc
            .perform(get("/api/_search/countries?query=id:" + country.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(country.getId().intValue())))
            .andExpect(jsonPath("$.[*].countryName").value(hasItem(DEFAULT_COUNTRY_NAME)));
